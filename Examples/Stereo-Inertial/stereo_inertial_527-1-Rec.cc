...
int main(int argc, char **argv)
{
    ...
    string f_file =  string(argv[argc-1]) + "CameraTrajectory.txt";
    string kf_file = string(argv[argc-1]) + "KeyFrameTrajectory.txt";
    if(argc < 7) 
    {
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            SLAM.TrackStereo(imLeft,imRight,tframe, f_file, kf_file, vImuMeas);
    ...
    SLAM.SaveAssociationInfo(string(argv[argc-1]) + "AssoInfo.txt");
    SLAM.SaveKPInfo(string(argv[argc-1]) + "KPInfo.txt");
    ...
    cout << "-------" << endl << endl;
    if ( tot_images % 2 )
        cout << "median single tracking time: " << vTimesTrack[tot_images/2] << endl;
    else
        cout << "median single tracking time: " << (vTimesTrack[tot_images/2] + vTimesTrack[tot_images/2-1])/2<< endl;
    cout << "mean single tracking time: " << totaltime/proccIm << endl;
...
/**
* @brief 导入图像 TUM-VI格式 
* @param[in] strPathLeft string 左目图像的路径 
* @param[in] strPathRight string 右目图像的路径
* @param[in] strPathTimes string 图像时间戳的路径  
* @param[in&Out] vstrImageLeft vector<string> 左目图像的名称 
* @param[in&Out] vstrImageRight vector<string> 右目图像的名称 
* @param[in&Out] vTimeStamps vector<double> 时间戳 
*/
void LoadImagesTUMVI(const string &strPathLeft, const string &strPathRight, const string &strPathTimes,
                vector<string> &vstrImageLeft, vector<string> &vstrImageRight, vector<double> &vTimeStamps)
{
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            string item2 = s.substr(pos + 1, pos2 - pos - 1);
            // 将string类型转换成double类型的函数
            double t = stod(item2);
...
* @param [in] strImuPath 
* @param [in&Out] vTimeStamps vector<double> 时间戳 
* @param [in&Out] vAcc vector<cv::Point3f> Acc 
* @param [in&Out] vGyro vector<cv::Point3f> Gyro 
*/
void LoadIMU(const string &strImuPath, vector<double> &vTimeStamps, vector<cv::Point3f> &vAcc, vector<cv::Point3f> &vGyro)
{
    ifstream fImu;

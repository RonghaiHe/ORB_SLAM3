...
double ttrack_tot = 0; // track总用时，初始化为0
int main(int argc, char **argv)
{
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            SLAM.TrackStereo(imLeft,imRight,tframe, f_file, kf_file);
                ...
                vector<string> &vstrImageLeft, vector<string> &vstrImageRight, vector<double> &vTimeStamps)
{
    ...
    {
        stringstream item;
        item << setw(10) << setfill('0') << i++;
        vstrImageLeft.push_back(strPathLeft + "/" + item.str() + ".jpg");
        vstrImageRight.push_back(strPathRight + "/" + item.str() + ".jpg");
